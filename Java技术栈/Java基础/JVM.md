...
# 八股
参考：[JVM面试题](https://heuqqdmbyk.feishu.cn/wiki/KjT8wE2HtiKkc7kjHUFczNULn4c)
...
TLAB是Java虚拟机的一种优化即使，是线程本地的内存分配缓冲区，每个线程都有自己独立的TLAB，也就是每个线程都有自己的一块内存空间，线程分配和操作对象的时候，都是会操作自己的TLAB，然后再同步到堆上。
...
一个Java对象在JVM中主要包含以下几个部分：
1. 对象头（Object Header）：
	- Mark Word：用于存储对象自身的运行时数据，比如哈希码、锁状态标识、线程持有的锁等等
	- Class Pointer（类型指针）：指向对象的类元数据
	- 数组长度（仅数组对象）
2. 实例数据
	- 存储实体类的实例字段内容
3. 对齐填充：JVM要求对象的起始地址是8字节的整数倍，因此可能在对象结尾填充对齐字节。
...
随着JVM发展，并不是所有的对象就一定在堆上分配：
1. 逃逸分析：如果确定一个对象不会被方法之外的代码访问，那么就在栈上分配对象
2. 标量替换：如果对象的所有属性可以独立处理，那么JVM会对对象进行标量替换，将对象分解为其基本类型的成员变量进行优化。
3. 寄存器分配：有些情况下，JIT（即时编译器）可能会将对象直接在寄存器上进行分配
...
**可达性分析法：Java使用的是可达性分析算法来判断对象是否可以被回收，Java使用的就是这种回收判断方法。**
可达性分析将对象分为两类：垃圾回收的根对象（GC Root）和普通对象，对象与对象之间存在引用关系。如果某个对象到GC Root是可达的，对象就不可被回收。

GC Root是一种特殊的引用，被认为是程序中可访问对象的起始点，也就是从这些对象开始，可以追踪到所有仍然被程序引用的对象。
哪些对象被称之为GC Root对象呢？
- 局部变量引用：方法中的参数或者定义在方法中的局部变量
+ 活动线程引用：正在运行的线程引用，线程本地存储的对象也是GC Root
+ 静态变量引用
+ 虚拟机类加载器引用
+ JNI引用（Java本地接口）创建的本地代码引用


### 垃圾回收标记对象是否可以被回收：三色标记
JVM（Java虚拟机）中的三色标记（Tri-color Marking）是一种用于垃圾回收（Garbage Collection, GC）的算法思想，特别是在并发标记阶段，用于追踪和管理对象的可达性。它是现代垃圾回收器（如CMS、G1等）实现增量式或并发垃圾回收的核心机制之一。

三色标记算法将对象分为三种颜色，用以表示对象的处理状态：
1. **白色（White）**：表示对象尚未被垃圾回收器访问，可能是垃圾，也可能是尚未检查的可达对象。在标记阶段开始时，所有对象都是白色。
2. **灰色（Grey）**：表示对象已经被垃圾回收器访问到，知道它是可达的，但它的引用对象（子节点）尚未完全处理。灰色对象是标记过程中需要进一步扫描的对象。
3. **黑色（Black）**：表示对象已被垃圾回收器完全处理，它是可达的，且它的所有引用对象都已经被标记为可达（即已处理为黑色或灰色）。

**工作原理：**
三色标记算法的核心目标是通过递归或迭代的方式，从**根对象（GC Roots）**开始，标记所有可达对象，最终将不可达的白色对象识别为垃圾，供回收。过程如下：
1. **初始标记**：从GC Roots（如栈上的引用、静态变量等）开始，将直接引用的对象标记为灰色。
2. **并发标记**：
    - 从灰色对象集合中取出一个灰色对象，检查它引用的其他对象。
    - 将这些被引用的对象标记为灰色，并将当前对象标记为黑色。
    - 重复此过程，直到灰色对象集合为空。
3. **结束标记**：此时，所有可达对象都被标记为黑色，剩余的白色对象即为垃圾，可以被回收。

在并发垃圾回收中（如CMS或G1），标记过程与应用程序线程（Mutator）同时运行，可能导致对象引用关系的变化。两个主要问题可能破坏标记的正确性：
1. **漏标（Missing Mark）**：
    - 如果一个黑色对象在标记过程中新增了对白色对象的引用，而这个白色对象未被重新标记为灰色，因为黑色对象已经被扫描过了，就可能被误认为垃圾而被回收。
    - **解决方法**：使用**写屏障（Write Barrier）**。当应用程序修改对象引用时，写屏障会记录这些变化（如将新引用的对象标记为灰色或记录引用关系），确保后续标记阶段能重新扫描这些对象。
2. **多标（Over Mark）**：
    - 某些对象可能在标记过程中被错误标记为可达，但实际上已不可达。这种情况不会导致内存泄漏，但可能降低回收效率。

**三色不变性（Tri-color Invariant）**
为了保证标记的正确性，三色标记算法遵循以下不变性：
- **强三色不变性**：黑色对象不能直接引用白色对象（否则可能导致漏标）。
- **弱三色不变性**：允许黑色对象引用白色对象，但这些白色对象必须通过灰色对象的引用路径可达。
写屏障和读屏障（Read Barrier）是维护三色不变性的常用技术。例如：
- **写屏障**：在对象引用更新时，将被引用的对象标记为灰色，或者记录引用关系。
- **读屏障**：在读取对象引用时，确保引用的对象被正确标记（较少使用）。
...
2. 软引用，软引用相对于强引用是一种比较弱的引用关系，**如果一个对象只有软引用关联到它，当程序内存不足时，就会将软引用中的数据进行回收。** 软引用主要在缓存框架中使用。通过`SoftReference`定义软引用

3.   弱引用，弱引用的整体机制和软引用基本一致，区别在于弱引用包含的对象在垃圾回收时，**不管内存够不够都会直接被回收，** 弱引用适合实现Map的缓存，通过`WeakReference`实现

4. 虚引用（幽灵引用/幻影引用），不能通过虚引用对象获取到包含的对象。**虚引用唯一的用途是当对象被垃圾回收器回收时可以接收到对应的通知。**  虚引用主要用于对象在垃圾回收时的一些清理工作。通过`phantomReference`实现，结合`ReferenceQueue`队列配合使用。
...
Servivor存放的是存活的对象，每经过一次minor gc，就会移动分区，然后年龄+1
划分两个区的核心目的：
1. 利用复制算法高效回收内存
2. 通过年龄分阶段筛选对象，减少老年代压力
3. 优化内存利用率，平衡性能与资源消耗
