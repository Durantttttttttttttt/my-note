...
![img](img/ShardingSphere5/362d22168bf344687ec0c206aa115807.jpg)
...
![image-20220804223138651](img/ShardingSphere5/image-20220804223138651.png)
...
- 在实际设计过程中，每个系统不可能只处理一种数据，而是包含多种类型的数据，`有的数据必须选择 CP，有的数据必须选择 AP，分布式系统理论上不可能选择 CA 架构。`
  ...
  ![img](img/ShardingSphere5/6e7d7bd54d7a4eb67918080863d354d7.png)
...
![img](img/ShardingSphere5/2ccafe41de9bd7f8dec4658f004310d6.png)
...
![img](img/ShardingSphere5/71f41d46cc5c0405f4d4dc944b4350c9.jpg)

下图展示了根据业务需要，将用户表和订单表垂直分片到不同的数据库的方案：
![image-20220804221855449](img/ShardingSphere5/image-20220804221855449.png)
...
![img](img/ShardingSphere5/136bc2f01919edcb8271df6f7e71af40.jpg)
...
![image-20220804222212087](img/ShardingSphere5/image-20220804222212087.png)
...
![image-20220804223321167](img/ShardingSphere5/image-20220804223321167.png)
...
**其基本架构是：** 以读写分离为例
![img](img/ShardingSphere5/f8d538f9201e3ebee37dfdcd1922e9df.jpg)

### 4.2、中间件封装
中间件封装指的是`独立一套系统出来`，实现读写操作分离和数据库服务器连接的管理。对于业务服务器来说，访问中间件和访问数据库没有区别，在业务服务器看来，中间件就是一个数据库服务器。
**基本架构是：** 以读写分离为例
![img](img/ShardingSphere5/2a2dba7f07581fd055d9cd5a3aa8388e.jpg)
...
![image-20220804195402870](img/ShardingSphere5/image-20220804195402870.png)
...
![image-20220804195432673](img/ShardingSphere5/image-20220804195432673.png)
...
![img](img/ShardingSphere5/image-20220714133617856.png)
...
- `step2：` 当slave上执行 `start slave或者start replica` 命令之后，slave会创建一个 `IO 线程`用来连接master，请求master中的binlog。
...
![image-20220807183231101](img/ShardingSphere5/image-20220807183231101.png)
...
![img](img/ShardingSphere5/0.08703112216569037.png)
...
![image-20220804191852164](img/ShardingSphere5/image-20220804191852164.png)
...
在**从机**上执行以下SQL操作
```sql
CHANGE MASTER TO MASTER_HOST='192.168.101.130', 
MASTER_USER='slave',MASTER_PASSWORD='123456', MASTER_PORT=3306,
MASTER_LOG_FILE='binlog.000008',MASTER_LOG_POS=1515; 
```
...
![img](img/ShardingSphere5/image-20220715000533951.png)
...
![img](img/ShardingSphere5/image-20220714235426120.png)



**典型的错误例如：**`Last_IO_Error: Got fatal error 1236 from master when reading data from binary log: 'Client requested master to start replication from position > file size'`（从二进制日志读取数据时，主服务器出现致命错误 1236：'客户端请求主服务器从位置 > 文件大小开始复制）
...
启动docker容器后提示 `WARNING: IPv4 forwarding is disabled. Networking will not work.`(IPv4 转发已禁用。网络将无法正常工作)
![img](img/ShardingSphere5/image-20220715004850504.png)
然不影响主从同步的搭建，但是如果想从远程客户端通过以下方式连接docker中的MySQL则没法连接
```shell
...
![image-20220807232456342](img/ShardingSphere5/image-20220807232456342.png)
...
![image-20220810163534068](img/ShardingSphere5/image-20220810163534068.png)
...
服务器规划：使用`docker`方式创建如下容器
![image-20220808033239206](img/ShardingSphere5/image-20220808033239206.png)
...
![image-20220819152009158](img/ShardingSphere5/image-20220819152009158.png)
...
```shell
docker run -d \
-v /usr/local/proxy-a/conf:/opt/shardingsphere-proxy/conf \
-v /usr/local/proxy-a/ext-lib:/opt/shardingsphere-proxy/ext-lib \
...
![image-20220819151154763](img/ShardingSphere5/image-20220819151154763.png)
