...
连接层：缓存连接池，分配连接server的连接，建立客户端到Server层的连接
服务层：包含查询缓存、解析器（解析器、预处理器、优化器）、执行器。解析优化SQL，确定执行方案
存储引擎层：执行SQL，存储数据
...
### 什么是索引下推
索引下推是一种执行优化机制，允许存储引擎在索引扫描就过滤掉不满足条件的记录，减少传输给Server层的开销
...
- 使用 ICP：存储引擎在索引扫描时直接应用条件 col2 > 10，只返回满足条件的记录。

## InnoDB存储引擎底层结构
Innodb逻辑结构：
![[IMG-20250601173721644.png]]
InnoDB的一个存储空间分为表空间、段、区、页、行
一个InnoDB的存储文件对应一个表空间，分为各个类型的段，段分为多个区，一个区占用1M，区分为多个类型的页page，每个页的大小是16k，且连续，页上就记录着数据库表中的记录行。

## MySQL锁的理解

### MySQL锁的分类
- 按照锁粒度分：全局锁、表锁、行锁、页锁、元数据锁
- 按照类型分为读锁和写锁（都是悲观锁）
- 按照性能分为乐观锁、悲观锁和意向锁
...
加锁方式：SELECT ... LOCK IN SHARE MODE（InnoDB）。

写锁（排它锁，X锁），排它锁指一个事务在修改数据的时候会加上排它锁，另外一个事务要想加任意类型的锁，必须等待排它锁释放。事务不是并行执行的。
加锁方式：SELECT ... FOR UPDATE、UPDATE、DELETE（InnoDB）。
...
全局锁：锁住整个数据库实例，通常用于备份或全局操作。
-  加锁方式：FLUSH TABLES WITH READ LOCK。
- 特点：所有表只读，无法写入，适合一致性备份。
- 使用场景：如使用mysqldump进行全库备份时。
- 注意：全局锁会阻塞所有写操作，慎用。

元数据锁：保护表的结构（如表定义、字段等），在执行DDL（如ALTER TABLE）或DML时自动加锁。
- 特点：MDL锁在事务提交或回滚时释放，防止表结构和数据操作冲突。
- 使用场景：如执行ALTER TABLE时会加MDL写锁，阻止其他事务访问该表。
...
示例：SELECT * FROM table WHERE id BETWEEN 10 AND 20 FOR UPDATE;会锁住id在10到20之间的间隙。
### 临键锁（Next-Key Locks）
临键锁是记录锁和间隙锁的组合，引入的主要目的是解决幻读，能够封锁相邻两个键之间的空白区域，包括该记录

示例：对id=10的记录加锁时，会同时锁住(5, 10]的范围（假设5是前一个索引值）。
...
红黑树（自平衡的二叉搜索树）的弊端：节点一多，树高还是高，且每个节点只存储一个数据，节点之间还是不连续的
...
### 什么叫覆盖索引？
覆盖索引是指在SELECT查询中，需要的数据全部能在非聚簇索引中的叶子节点找到，只使用二级索引就行了，避免了回表查询，提高了性能。使用覆盖索引可以减少对主键索引的查询次数，提高查询效率。
...
- 索引列不能存储Null值，使用not null约束它，当优化器知道每一列是否包含Null值时，可以更好确定使用哪个索引
...
注意：
- mysql会**一直向右匹配**直到遇到**范围查询**（>、<、between、like）就**停止匹配**，但是使用>= 或者 <=不会失效
...
15. distinct过滤的列要少，distinct * 远远比 distinct name要速度慢多了
...
2. 从库连接主库
3. 主库发送二进制日志给从库
4. 从库接收二进制日志并且写入到中继（relay log）日志中
