...
String键值对中的值可以是字符串、整数或浮点数
List 是一个有序的双向链表，元素可以重复，适合存储有序数据集合。
Hash 是一个键值对集合，适合存储结构化对象，键是字符串，值可以是字符串、整数等。
Set 是一个无序、不重复的字符串集合，适合存储唯一元素集合。
Sorted Set 是一个有序、不重复的字符串集合，每个元素关联一个浮点数分数（score），按分数排序。
...
缓存穿透是指**一直访问一个不存在的数据，这样会导致一直不走redis，直接访问DB，这样就会导致DB的压力，缓存没有作用，直至挂掉。** 

解决方法有**缓存空对象和布隆过滤器**两种方法解决，推荐使用布隆过滤器解决缓存穿透问题。
...
可以设置过期时间防止长期占用锁。但是有锁误删的问题和锁过期时间问题

关于锁过期问题：就是锁时间到了，但是业务还没有执行完：
1. 可以延长锁的时间
2. 续锁
...
当缓存中的**热点数据**突然过期了，如果这个数据还没有从DB中回传到redis中去建立缓存，这个时候如果有大量的并发请求过来，这些请求都会去请求数据库，瞬间吧DB压垮。
所以缓存击穿就是**热点缓存失效**的时候，大量的请求突然发送过来，由于缓存没有，所以都打到数据库上，造成数据库压力

解决方法有两个，
- 一种是使用**setnx来设置互斥锁**，当redis中的数据失效，会先通过setnx设置一个互斥锁，在重建redis数据之前的其他请求都只能通过get重试，重建redis数据成功后解锁。
- 第二种方法是使用逻辑过期，存储redis数据的时候**设置一个逻辑过期时间**，取出数据的时候判断这个逻辑过期是否到达了过期时间，过期时间到了开启一个新线程去重建redis数据，这个时候其他请求仍然可以去访问redis数据，但是是以前的旧数据。
- 第三种方案，是额外的啊，设置热点数据的过期时间为永不过期或者长时间不过期
...
缓存雪崩是值大量的key失效了或者缓存宕机了，大量的请求就直接请求到数据库了，导致数据库负载大而崩溃。

解决方法：
1. 合理的过期时间：为了防止大量的key失效，设置key的过期时间为随机值，分散失效时间
2. 多级缓存：使用本地缓存+分布式缓存，提高系统容错能力
3. 使用互斥锁：保证同一时间只有一个线程查询数据库，其他线程等待结果（鸡肋）
4. 高可用架构：使用Redis主从架构或者集群增加集群可用性
...
解决双写一致性的方案：
1. 延时双删，因为更新数据库和删除缓存后，读操作会导致可能读取的不是最新的数据，建立了一个旧缓存，所以设定一段时间内，再删除一次缓存，第二次删除操作可以消除缓存中可能存在的旧数据，从而提高数据一致性。
2. 更新数据库后，删除缓存如果多次失败，将要删除缓存的key发给消息队列，消息队列消费消息，重试删除缓存，直到成功。缺点有业务代码侵入
3. 使用MQ监听binlog日志异步删除缓存，使用专门的服务监听binlog的变化，如果有数据更新，通过监听器监听到变更，异步删除缓存

实际应用：
使用阿里开源框架Canal，对MySQL的binlog进行订阅，而Canal正式模仿了mysql的slave数据库的备份请求。
MQ消息中间件可以使用RocketMQ实现推送。
...
但是怕更新缓存后删除缓存的操作，其中存在一次读缓存建立缓存， 此时建立缓存会覆盖删除缓存后的操作，建立了一个旧缓存。
...
RDB会对Redis内存创建快照（内存数据的二进制序列化形式），创建快照之后，从库可以根据快照执行，得到相同的内存数据。
RDB是默认的持久化方式，RDB提供两个命令生成快照文件：save和bgsave
save和bgsave的区别：
- save：阻塞当前Redis进程，直到RDB持久化过程完成
- bgsave：主线程fork创建子线程，由子线程完成持久化，阻塞时间很短
配置自动触发RDB创建：
```java
save 3600 1 -- 3600秒内有一个key被修改，触发RDB替换
save 600 100 -- 600秒内有100key被修改，触发RDB替换
save 60 10000 -- 60秒内有10000key被修改，触发RDB替换
```
shutdown触发RDB持久化：redis命令行指向shutdown也会触发RDB
flushall触发RDB持久化：flushall清空所有数据库中的数据，触发RDB

RDB持久化的优点：
- 使用二进制保存内存快照，恢复速度快
- 二进制保存，相对于AOF，文件体积小
缺点：
- 是定期保存RDB，可能会存在最近的操作丢失


AOF持久化记录每一条更改数据的命令，命令会先到AOF缓冲区，再根据配置的持久化机制同步到磁盘上面AOF文件。
AOF开启：在redis配置文件中添加配置
```java
appendonly yes
aof-use-rdb-preamble no --关闭RDB+AOF混合模式
```
关于内核缓冲区持久化到磁盘的策略有三种：
1. appendfsync always，主线程会等到持久化磁盘完毕
2. appendfsync everysec，主线程写到缓冲区就返回，持久的操作交给后台线程，每秒刷新一次缓冲区中的数据到AOF文件中
3. appendfsync no，主线程写到缓冲区返回，持久操作交给操作系统，不推荐
AOF是执行完命令之后才写入缓冲区的，这样会造成数据的丢失，但是效率高。
AOF重写机制：AOF文件会将原本大的AOF文件重写一个AOF文件，新的AOF文件体积更小，因为对命令进行了压缩。
自动重写配置：
```java
auto-aof-rewrite-percentage 100 --当AOF文件体积达到上次重写之后的体积的100%触发重写
auto-aof-rewrite-min-size 64mb 当AOF文件超过64mb重写
```
手动重写命令：bgrewriteaof

AOF持久化优点：
- 数据可靠：因为是记录每一个执行的命令，不是像RDB的自动备份机制
- 可以保留写命令历史：AOF是一个追加日志文件，可以用于回放过去的写操作
缺点：
- 文件大，写命令比二进制更大
- 恢复速度慢，Redis重启需要执行整个AOF文件，速度较慢


混合持久化，Redis4.0支持混合持久化
开启：
```java
...
- RDB是备份数据文件，针对Redis内存做快照，把redis内存的数据写入到磁盘上，redis恢复数据可以从RDB快照文件中恢复数据。
...
- AOF比RDB数据安全性高，虽然RDB有bgsave不会堵塞主线程，但是还是会造成较大的浪费，因为RDB是定时备份，AOF取决于持久化策略，效率比RDB高，安全性也高。RDB是二进制文件，如果是Redis版本迭代，会存在不兼容的问题。AOF保存的是操作命令，更适合导出分析。

### 两种方式持久化哪种恢复的快？
RDB因为是二进制文件，保存时体积也比较小，所以它恢复得比较快。但它有可能会丢数据，因为它是定时自动备份，在没有备份之前可能会导致数据的丢失。
...
Redis中提供了两种数据过期策略。
1. 第一种是采用**惰性删除**，redis中的数据尽管达到了过期时间也不会被删除，只有在操作键的时候才去检查过期时间，如果过期删除它，反之返回它。好处就是节省CPU资源，但是会浪费内存空间，因为过期了还在内存中。
2. 第二种**定期删除**，每隔一段时间（默认是100ms）随机检查那些设置了过期时间的键，删除过期时间的键
...
## 分布式锁
### Redis中的分布式锁如何实现？
分布式锁是指在分布式系统中，确保在同一个时刻只能有一个节点对共享资源进行操作，保证分布式环境下并发控制中数据的一致性。
实现分布式锁的方案：
1. setnx+expire，通过setnx抢锁，再通过expire设置过期时间。缺点是抢锁和设置过期时间不是个原子性操作。
2. setnx+value(系统时间+过期时间)，解决1中的问题，缺点过期时间要在多个客户端中同步很难
3. 使用lua脚本（包含setnx+expire），保证原子性
4. 使用setnx扩展命令，setnx的时候指定过期时间，保证原子性。但是问题是：
	1. 锁过期释放了，业务没执行完。可以延长锁时间，或者续锁。
	2. 锁被其他线程误删。解决方法存储线程id，在释放锁判断，但是还有问题，不是一个原子操作，判断线程和删锁这个两个动作。
5. Redisson框架：方案四中的锁过期释放，这里提供一个WatchDog看门狗机制，每隔10秒检查是否还持有锁，持有就不断的延长时间
6. 前面都是基于单机版本的，对于分布式集群。RedLock+Redisson。RedLock机制：
	1. 对其他结点请求加锁，如果大于或者等于一半的结点加锁成功，并且使用的时间小于锁的有效期，这加锁成功
	2. 如果获取锁失败，解锁


### 那如何控制分布式锁的有效时长？
redis的`SETNX`指令不好控制这个问题。
我们当时采用的是redis的一个框架Redisson实现的。在Redisson中需要手动加锁，并且可以控制锁的失效时间和等待时间。当锁住的一个业务还没有执行完成的时候，Redisson会引入一个**WatchDog看门狗机制**。就是说，每隔一段时间就检查当前业务是否还持有锁。如果持有，就增加加锁的持有时间。当业务执行完成之后，需要使用释放锁就可以了。
...
### Redisson实现的分布式锁能解决分布式的主从一致性问题吗？
...
我们可以利用Redisson提供的**红锁**来解决这个问题，它的主要作用是，不能只在一个Redis实例上创建锁，应该是在多个Redis实例上创建锁，并且要求在大多数Redis节点上都成功创建锁，红锁中要求是Redis的节点数量要过半。这样就能避免线程1加锁成功后master节点宕机导致线程2成功加锁到新的master节点上的问题了。

但是，如果使用了红锁，因为需要同时在多个节点上都添加锁，性能就变得非常低，并且运维维护成本也非常高，所以，我们一般在项目中也不会直接使用红锁，并且官方也暂时废弃了这个红锁。如果非要保证数据的强一致性，**建议采用ZooKeeper实现的分布式锁**。


### 如果非要保证分布式锁的强一致性，这个怎么实现？
...
幂等操作是指多次操作的结果和一次执行的结果相同，多种操作会导致数据重复等问题。
解决幂等性方案：
1. 数据库唯一约束实现幂等性，确保主键的唯一
2. 乐观锁实现幂等性，加上版本标识
3. 分布式锁，通过分布式的排它锁
4. 获取token，根据请求生成token，服务端校验token判断请求是否重复

### 哨兵机制
Redis主从模式主要实现数据的冗余备份和读分担，并不会提供高可用性。
Redis哨兵的作用：
- 监控主从节点的状态
- 当主节点发生故障，自动的故障转移

哨兵机制原理：通过部署独立的哨兵节点，哨兵会监控主从节点的状态，并在发现故障时候自动完成故障发现和转移，实现高可用。
1. 哨兵选举：每个哨兵节点都可以成为领导者，发起选举后，如果哨兵的票数打到了一半，将成为领导
2. 监控主从节点：哨兵发送周期性命令检查主从节点健康状态，如果发现不可用，会触发一次故障转移。
3. 故障转移：主节点不可用，从其他从节点中选择一个新的主键，其他的从节点切换到新的主节点中
4. 客户端重定向：通知客户端新的主节点的位置
...
1. 可以设置最少的slave节点个数，比如设置至少要有一个从节点才能同步数据，
...
## Redis事务
Redis也支持事务，但是和关系型数据库事务不一样。
常用命令：
- multi：开启事务，客户端可以继续发送多条命令，但是这些命令不会立即执行，而是被放入一个队列中
- exec：执行队列中的所有命令
- discard：中断当前事务，情况事务队列并放弃执行事务
- watch key1 key2...：见识一个或者多个key，如果这些key在事务执行之前有改动，那么事务终止。

## Redis和Memcached的区别？

数据结构上，Redis支持的类型丰富，memcached只支持value为String类型。
持久化上，Redis支持持久化，memcache只支持内存缓存，断电就丢失内存数据
Redis支持lua脚本，原子操作，memcache没有

## Redis为什么用单线程？为什么后面又引入多线程

Redis单线程指的是网络IO和键值对读写都是通过单线程完成的。
为什么不用多线程：多线程能带来IO的提升，但是会导致共享资源和并发控制非常复杂，线程的切换也有性能损耗。还有一个原因是内存的操作本来就比较块，多线程又要竞争锁，没必要，省去开销时间

引入多线程是因为对网络IO进行多线程改造，多路复用的IO模型本质上还是同步阻塞的IO，使用多线程可以解决网络IO的等待问题
...
大Key是指那些占用内存空间特别大的键值。
大Key产生的原因：
1. 大型数据结构存储：存储大型字符串、列表、哈希表或者集合时候，如果这些数据结构的大小超过了一定阈值，就会产生大Key
2. 缓存滥用： 将大量数据作为单个键值缓存，没有分解
3. 数据导入：导入数据，数据量大的按照单个键值存储
4. 应用设计不佳：没有考虑Redis键值的大小限制
5. 数据累积：某些键值不断累积数据，导致超出预期

找出大Key的方法：
1. SCAN命令：不阻塞Redis的情况下遍历所有的Key，结合其他命令识别出大Key（如STRLEN、LLEN、SCARD等）
2. bigkeys参数：连接Redis服务的时候加上该参数，扫描大Key
```java
redis-cli -h 127.0.0.1 -p 6379 -bigkeys
```
3. Redis RDB Tools工具：分析RDB文件，扫描大Key
